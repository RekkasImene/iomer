...
void main() {
  ...
  group("test ArticleDao", () {
    test("articles can be created", () async {
      var article = Article(IDARTICLE: 101010, CODEARTICLE: "CODEARTICLE", LIBELLEARTICLE: "LIBELLEARTICLE", QTEARTICLE: 10);
  ...
  group("test CategorieDao", () {
    test("categories can be created", () async {
      var categorie = Categorie(IDCATEGORIE: 101010, CODECATEGORIE: "CODECATEGORI", LIBELLECATEGORIE: "LIBELLECATEGORIE", IDSITE: 1);
  ...
  group("test ConfigDao", () {
    test("configs can be created", () async {
      var config = Config(IDSITE: 101010, IDORIGINE: 1010, CODEORIGINE: "CODEORIGINE", CODEPOCKET: "CODEPOCKET", LIBELLEORIGINE: "LIBELLEORIGINE", NOMPOCKET: "NOMPOCKET");
  ...
  group("test DocumentDao", () {
    test("documents can be created", () async {
      var document = Document(IDATTACHEMENT: 101010, ATTACHEMENT: Uint8List.fromList([01,02,03,05]));
  ...
  group("test EquipementDao", () {
    test("equipements can be created", () async {
      var equipement = Equipement(IDEQUIPEMENT: 101010, CODEEQUIPEMENT: "CODEEQUIPEME", LIBELLEEQUIPEMENT: "LIBELLEEQUIPEMENT", IDSITE: 1010);
      await db.equipementDao.insertEquipement(equipement);
      List<Equipement> equipementDb = await db.equipementDao.getAllEquipements();
  ...
  group("test MatriculeDao", () {
    test("matricules can be created", () async {
      var matricule = Matricule(IDMATRICULE: 101010, CODEMATRICULE: "CODEMATRICUL", NOMMATRICULE: "NOMMATRICULE", PRENOMMATRICULE: "PRENOMMATRICULE", CHECKED: false);
  ...
  group("test ReservationDao", () {
    test("reservations can be created", () async {
      var reservation = Reservation(IDARTICLE: 101010, QTEARTICLE: 10, LIBELLEARTICLE: 'LIBELLEARTICLE', IDPIECE: 1010);
      await db.reservationDao.insertReservation(reservation);
      List<Reservation> reservationDb = await db.reservationDao.getAllReservations();
  ...
  group("test TacheDao", () {
    test("taches can be created", () async {
      var tache = Tache(IDTACHE: 101010, CODETACHE: "CODETACHE", LIBELLETACHE: "LIBELLETACHE", STATUTTACHE: 1);
